...
# # Start the server
# RUST_LOG=info cargo run -- server --listen-address "/ip4/127.0.0.1/udp/33333/quic-v1" --quic --node-key "secret" > /dev/null 2>&1 &
...
for bytes in $VALUES; do
    ...
    OUTPUT=$(RUST_LOG=info cargo run -- client --server-address "/ip4/127.0.0.1/udp/33333/quic-v1/p2p/12D3KooWBpZHDZu7YSbvPaPXKhkRNJvR7MkTJMQQAVBKx9mCqz3q" --quic --upload-bytes $bytes --download-bytes $bytes | grep bandwidth)
